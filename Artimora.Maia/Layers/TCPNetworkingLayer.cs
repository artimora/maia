...
namespace Artimora.Maia.Layers
{
    ...
    public sealed class TCPNetworkingLayer : NetworkLayer
    {
        ...
        private void TickClientRead()
        {
            ...
            try
            {
                var maxSize = hasClientOptions ? clientOptions.MaxMessageSize : int.MaxValue;
        ...
        )
        {
            ...
            while (buffer.Length - offset >= 4)
            {
                ...
                if (maxMessageSize > 0 && msgLen > maxMessageSize)
                    throw new InvalidOperationException($"Frame too large ({msgLen} bytes).");
